...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const row = new MessageActionRow()
      .addComponents(
        new MessageSelectMenu()
          ...
          .setMinValues(1)
          .setMaxValues(1)
          .setPlaceholder('Lucy Radio Stations')
          .addOptions([
            {
              label: 'Anime Lucy',
              value: 'anime',
    ...
    collector.on('collect', async i => {
      ...
      if (options === 'anime') {
        const ani = new MessageEmbed()
          .setColor(client.embedColor)
          .setDescription('<:radio:1119915830344437790> Current Station: **Anime Lucy** ')

        if (ress) {
          ress.oldradio = station
          ress.Radio = 'Anime Lucy'
          await ress.save()
          if (!m) return
          m.edit({
            embeds: [ani]
      ...
      if (options === 'sleep') {
        const slp = new MessageEmbed()
          .setColor(client.embedColor)
          .setDescription('<:radio:1119915830344437790> Current Station: **Sleep Lucy** ')

        if (ress) {
          ress.oldradio = station
          ress.Radio = 'Sleep Lucy'
          await ress.save()
          if (!m) return
          m.edit({
            embeds: [slp]
      ...
      if (options === 'study') {
        const sty = new MessageEmbed()
          .setColor(client.embedColor)
          .setDescription('<:radio:1119915830344437790> Current Station: **Study Lucy** ')

        if (ress) {
          ress.oldradio = station
          ress.Radio = 'Study Lucy'
          await ress.save()
          if (!m) return
          m.edit({
            embeds: [sty]
