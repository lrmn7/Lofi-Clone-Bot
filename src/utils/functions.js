...
async function oops (channel, args) {
  try {
    const embed1 = new MessageEmbed().setColor('RED').setDescription(`${args}`)
...
async function playerhandler (query, player, message) {
  ...
  const d = await db.findOne({ Guild: message.guildId })
  const n = new MessageEmbed().setColor(message.client.embedColor)
  ...
  if (result.type === 'PLAYLIST') for (const track of tracks) player.queue.add(track)
...
async function trackStartEventHandler (msgId, channel, player, track, client) {
  try {
    ...
    if (!message) {
      const embed1 = new MessageEmbed().setColor(client.embedColor).setDescription(`[${track.title}](${track.uri}) - \`[ ${track.isStream ? '[**◉ LIVE**]' : convertTime(player.queue.current.length)} ]\``).setImage(icon).setFooter({ text: `Requested by ${player.queue.current.requester.tag}`, iconURL: player.queue.current.requester.displayAvatarURL({ dynamic: true }) })
